    ...
    try {
      if(state === 'Login'){
        ...
        if(data.success){
          ...
          //we have to get the token from the response and set it in appcontext
          setToken(data.token)
          setUser(data.user)
          //to store token in browser's local storage
          localStorage.setItem('token', data.token)
          //next we have to hide the login form
          setShowLogin(false)
        }else{
          //we will display one error message in toast notifcation, for which we need toastify package
          toast.error(data.message)

        }
      }
      //if user not logged in we have to call registration
      else{
        ...
        if(data.success){
          ...
          //next we have to hide the registration form
          setShowLogin(false)
        }else{
          //we will display one error message in toast notifcation, for which we need toastify package
          toast.error(data.message)
...
/* to disable scrolling the page */ 
  useEffect(()=>{
    document.body.style.overflow = 'hidden';
/*to enable scrolling once login is done or login form is closed*/
    return ()=>{
  ...
  return (
    ...
    className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        <p className='text-sm text-center'>{state === 'Login' ? 'Welcome back! Please sign in to continue' : 'Please sign up to create an account'}</p>
